...
def update_view(request,pk):
    ...
    if user.unit=='python':
        ...
        queryset=Javascript.objects.get(pk=pk)
        form=jsUpdateForm(instance=queryset)
        if request.method=='POST':
            form=jsUpdateForm(request.POST,instance=queryset)
            if form.is_valid():
                form.save()
                return redirect('/grades/')
        context={'form':form}
        ...
        queryset=Sql.objects.get(pk=pk)
        form=sqlUpdateForm(instance=queryset)
        if request.method=='POST':
            form=sqlUpdateForm(request.POST,instance=queryset)
            if form.is_valid():
                form.save()
                return redirect('/grades/')
        context={'form':form}
        ...
        queryset=Php.objects.get(pk=pk)
        form=phpUpdateForm(instance=queryset)
        if request.method=='POST':
            form=phpUpdateForm(request.POST,instance=queryset)
            if form.is_valid():
                form.save()
                return redirect('/grades/')
        context={'form':form}
        return render(request,'entry.html',context)
def updateatt_view(request,pk):
    pk2=request.session.get('pk')
    print(pk)
    user=CustomUser.objects.get(pk=pk2)
    if user.unit=='python':
        queryset=pythonAttendance.objects.get(pk=pk)
        form=pyattendaceForm(instance=queryset)
        if request.method=='POST':
            form=pyattendaceForm(request.POST,instance=queryset)
            if form.is_valid():
                form.save()
        ...
        queryset=jsAttendance.objects.get(pk=pk)
        form=jsattendaceForm(instance=queryset)
        if request.method=='POST':
            form=jsattendaceForm(request.POST,instance=queryset)
            if form.is_valid():
                form.save()
                return redirect('/viewatt/')
        context={'form':form}
        ...
        queryset=sqlAttendance.objects.get(pk=pk)
        form=sqlattendaceForm(instance=queryset)
        if request.method=='POST':
            form=sqlattendaceForm(request.POST,instance=queryset)
            if form.is_valid():
                form.save()
                return redirect('/viewatt/')
        context={'form':form}
        ...
        queryset=phpAttendance.objects.get(pk=pk)
        form=phpattendaceForm(instance=queryset)
        if request.method=='POST':
            form=phpattendaceForm(request.POST,instance=queryset)
            if form.is_valid():
                form.save()
                return redirect('/viewatt/')
        context={'form':form}
...
def notifications_view(request):
    ...
    if user.unit=='python':
        ...
        for student in students:
            ...
            if student.hours<30 and clubs>1:
            ...
            elif student.hours>30 and clubs>1:
                subject = 'Python class fail report'
...
def download_attendance(request):
    ...
    if user.unit=='python':
        ...
        writer=csv.writer(response)
        students=pythonAttendance.objects.all()
        writer.writerow(['Reg','Name','From(hrs)','To(hrs)','Date','Present'])
        for student in students:
            writer.writerow([student.reg,student.name,student.From_hrs,student.To_hrs,':'+str(student.date),student.present])
        return response
    elif user.unit=='javascript':
        response=HttpResponse(content_type='text/csv')
        response['Content-Disposition']='attachment; filename=jsattendance.csv'
        writer=csv.writer(response)
        students=jsAttendance.objects.all()
        writer.writerow(['Reg','Name','From(hrs)','To(hrs)','Date','Present'])
        for student in students:
            writer.writerow([student.reg,student.name,student.From_hrs,student.To_hrs,':'+str(student.date),student.present])
        return response
        ...
        writer=csv.writer(response)
        students=sqlAttendance.objects.all()
        writer.writerow(['Reg','Name','From(hrs)','To(hrs)','Date','Present'])
        for student in students:
            writer.writerow([student.reg,student.name,student.From_hrs,student.To_hrs,':'+str(student.date),student.present])
        return response
        ...
        writer=csv.writer(response)
        students=phpAttendance.objects.all()
        writer.writerow(['Reg','Name','From(hrs)','To(hrs)','Date','Present'])
        for student in students:
            writer.writerow([student.reg,student.name,student.From_hrs,student.To_hrs,':'+str(student.date),student.present])
        return response
...
def staffpage_view(request):
    t=CustomUser.objects.filter(username__startswith='lecturer')
    activeusers=[]
    dusers=[]
    for v in t:
        if v.is_active==True:
            activeusers.append(v)
        else:
            dusers.append(v)
    context={
        'staff':CustomUser.objects.filter(username__startswith='lecturer'),
...
def vatt_view(request):
    pk=request.session.get('pk')
    print(pk)
    user=CustomUser.objects.get(pk=pk)
    if user.unit=='python':
        print('continue!!!!!')
        attendance=pythonAttendance.objects.all()
        context={
            'att':attendance
        }


        return render(request, 'vatt.html',context)
    elif user.unit=='php':
        print('continue!!!!!')
        attendance=phpAttendance.objects.all()
        context={
            'att':attendance
        }
        ...
        print('continue!!!!!')
        attendance=jsAttendance.objects.all()
        context={
            'att':attendance
        }
        ...
        print('continue!!!!!')
        attendance=sqlAttendance.objects.all()
        context={
            'att':attendance
        }
...
def deactivate(request,pk):
    ...
    user.save()
    messages.success(request, str(user.username) +' successfully disabled.')
    return redirect('/staffpage/')
def activate(request,pk):
    user = CustomUser.objects.get(pk=pk)
    user.is_active = True
    user.save()
    messages.success(request, str(user.username) +' successfully activated.')
    return redirect('/staffpage/')